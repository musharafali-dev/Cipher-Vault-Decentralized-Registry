    ...
    <header className="sticky top-0 z-50 w-full glass-panel border-b border-white/10 backdrop-blur-xl">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-20 flex items-center justify-between">
        ...
        <Link href="/" className="flex items-center gap-3 group cursor-pointer">
          <div className="relative w-11 h-11 rounded-2xl bg-gradient-to-tr from-amber-500 via-amber-600 to-emerald-500 p-0.5 shadow-glowGold group-hover:scale-105 transition-all duration-300">
            <div className="w-full h-full bg-darkSurface rounded-[14px] flex items-center justify-center">
              <Shield className="w-5 h-5 text-amber-400 group-hover:rotate-12 transition-transform duration-300" />
            </div>
          </div>
          <div className="flex flex-col">
            <span className="font-extrabold text-xl text-slate-100 tracking-tight flex items-center gap-1.5 font-heading">
              Cipher<span className="text-amber-400">Vault</span>
            </span>
        ...
        <nav className="hidden md:flex items-center gap-1 bg-darkSurface/90 border border-white/10 p-1.5 rounded-2xl backdrop-blur-md">
          {navLinks.map((link) => {
          ...
          {isConnected && address ? (
            <div className="relative">
              <button
                ...
                className="flex items-center gap-3 vault-card px-4 py-2 rounded-2xl border border-amber-500/30 hover:border-amber-400/60 transition-all duration-300 cursor-pointer"
              >
              ...
              {/* Dropdown Menu */}
              {dropdownOpen && (
                <div className="absolute right-0 mt-3 w-72 vault-card rounded-3xl p-3 shadow-2xl border border-white/15 flex flex-col gap-1 z-50 animate-fadeIn">
                  ...
                  <button
                    ...
                    className="flex items-center justify-between p-3 hover:bg-rose-500/10 text-rose-400 rounded-2xl text-xs font-bold transition cursor-pointer"
                  >
            ...
            <button
              ...
              className="vault-button-gold text-sm"
            >
              {isConnecting ? (
                <>
