  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 bg-grid-pattern">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 pb-8 mb-8 border-b border-white/10">
        <div>
          <h1 className="text-3xl sm:text-4xl font-extrabold text-slate-100 flex items-center gap-3 font-heading">
            <Database className="w-8 h-8 text-amber-400" />
            <span>Decentralized Vault Dashboard</span>
          </h1>
        ...
        <div className="flex items-center gap-3">
          <button
            ...
            disabled={isLoading}
            className="p-3 rounded-2xl vault-card hover:border-amber-500/40 text-slate-300 hover:text-white transition cursor-pointer"
          ...
          {isConnected ? (
            <button
              ...
              className="vault-button-gold text-sm cursor-pointer"
            >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5 mb-8">
        <div className="vault-card-hover rounded-3xl p-6">
          <p className="text-xs text-slate-400 uppercase tracking-wider font-semibold">Active Wallet</p>
          <p className="text-sm font-mono font-bold text-amber-400 mt-1.5 truncate">
      ...
      <div className="vault-card rounded-3xl p-5 mb-8 flex flex-col md:flex-row items-center justify-between gap-4">
        {/* Search */}
      ...
      {/* Record Cards */}
      {isLoading ? (
        <div className="vault-card rounded-3xl p-16 text-center flex flex-col items-center justify-center">
          <Loader2 className="w-10 h-10 text-amber-400 animate-spin mb-4" />
          <p className="text-base text-slate-200 font-semibold font-heading">Fetching smart contract state from blockchain...</p>
        </div>
      ) : filteredRecords.length === 0 ? (
        <div className="vault-card rounded-3xl p-16 text-center flex flex-col items-center justify-center">
          <FileCode className="w-14 h-14 text-slate-500 mb-4" />
          <h3 className="text-xl font-bold text-slate-200 font-heading">No Records Found</h3>
          ...
          {isConnected && (
            <button onClick={() => setIsCreateOpen(true)} className="vault-button-gold text-xs px-5 py-2.5 cursor-pointer">
              Mint First Record
            </button>
          )}
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredRecords.map((record) => {
            ...
            return (
              <div key={record.id} className="vault-card-hover rounded-3xl p-6 flex flex-col justify-between cursor-pointer">
                <div>
                  {/* Category & Status */}
                  <div className="flex items-center justify-between gap-2 mb-4">
                    <span className="px-3 py-1 rounded-xl text-xs font-semibold badge-gold flex items-center gap-1.5">
                      <Tag className="w-3.5 h-3.5" />
                      {record.category}
                    </span>
                    {isOwner ? (
                      <span className="px-2.5 py-0.5 rounded-lg text-[10px] font-mono badge-emerald font-bold">
                        Owner
                      </span>
                    ) : (
                  ...
                  {/* Title */}
                  <h3 className="font-extrabold text-lg text-slate-100 mb-3 leading-snug line-clamp-2 font-heading">
                    {record.title}
                  </h3>
                  ...
                  <div className="p-3 bg-darkBg/90 rounded-2xl border border-white/5 mb-5 shadow-inner">
                    ...
                    <div className="flex items-center justify-between gap-2 font-mono text-xs text-amber-300">
                      ...
                      <button
                        ...
                        className="p-1.5 hover:text-white text-slate-400 transition hover:bg-white/10 rounded-lg cursor-pointer"
                        title="Copy Hash"
                      >
                  ...
                  {isOwner && (
                    <div className="flex items-center gap-1.5">
                      <button
                        ...
                        className="p-2 hover:bg-amber-500/20 hover:text-amber-300 rounded-xl transition border border-transparent hover:border-amber-500/30 cursor-pointer"
                        title="Edit Record"
                      >
                      ...
                      <button
                        ...
                        className="p-2 hover:bg-rose-500/20 hover:text-rose-400 rounded-xl transition border border-transparent hover:border-rose-500/30 cursor-pointer"
                        title="Delete Record"
                      >
